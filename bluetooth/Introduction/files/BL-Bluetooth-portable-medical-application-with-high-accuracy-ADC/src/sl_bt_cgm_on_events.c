...
static sl_simple_timer_t cgm_fast_adv_timer;
static bool fast_adv = true;
static void sl_bt_create_user_adv(void);

static sl_simple_timer_t measurement_timer;

/**************************************************************************//**
 * CGM fast advertiser 30s oneshot timer stop
 *****************************************************************************/
static void sl_cgm_fast_adv_timeout(sl_simple_timer_t *handle, void *data)
{
  sl_status_t sc = SL_STATUS_FAIL;
  (void)&handle;
  (void)&data;
  // Stop timer.
  sc = sl_simple_timer_stop(&cgm_fast_adv_timer);
  if(sc != SL_STATUS_OK){
      app_log_info("stop bgm_fast_adv_timer failed 0x%04X\n", sc);
  }
  fast_adv = false;
  ...
  sc = sl_bt_advertiser_set_timing(
                        ...
                        16384, // max. adv. interval Value in units of 0.625 ms
                        0,   // adv. duration
...
static void sl_bt_create_user_adv(void)
{
  ...
  const uint8_t value[] = {0x02,0x01,0x06,0x03,0x03,0x1F,0x18,0x0B,0x08,0x73,\
                           0x69,0x6C,0x61,0x62,0x73,0x2D,0x63,0x67,0x6D,0x03, \
                           0x19,0x0D,0x00,0x07,temp,address.addr[0], \
                           address.addr[1],address.addr[2],address.addr[3], \
                           address.addr[4],address.addr[5]};
  sc = sl_bt_legacy_advertiser_set_data(advertising_set_handle, 0,sizeof(value),value);
  app_assert_status(sc);
  ...
  if(fast_adv){
    ...
    sc = sl_simple_timer_start(&cgm_fast_adv_timer,
...
static void sl_bt_connection_closed_cb(uint16_t reason)
{
  ...
  // do not Stop cgm_periodic_timer.
...
void sl_bt_cgm_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_system_boot_id:{
      ...
      sl_simple_timer_start(&measurement_timer,
