[English](Zigbee-Introduction-of-Zigbee-Basic.md) | Chinese
...
众所周知，在物联网行业中，我们有两个典型的网络。一个是WAN（广域网），另一个是PAN（个人区域网）。
...
从一开始，Silicon Labs就致力于Zigbee技术。Silicon Labs的Zigbee技术来自Ember，Ember是一家自成立以来就开始研究Zigbee的公司。2012年，Silicon Labs收购了Ember，并继续提供Zigbee产品和解决方案。
...
Zigbee联盟的主要三个工作是：
- 为IoT（物联网）的无线设备到设备通信**制定**开放的全球标准
...
自2017年起强制Zigbee 3.0，因为符合先前规范的产品不会获得认证。
...
4. 应用层留给用户设计。每个应用程序实例称为一个端点。为管理功能保留了一个特殊的端点，即端点0。我们也将此管理功能模型称为Zigbee设备对象（ZDO）。
5. 在APS层和网络层中，有一些安全功能可用于保护网络免遭黑客攻击。

## 2. 物理层
Zigbee在ISM频率上工作。通道定义如下：
...
### 2.2. 输出功率
802.15.4是专为低功耗，低数据速率网络而设计的，其目标是低成本。这些通常称为PAN或个人局域网。这里的想法是这些将低于中等的无线电范围应用设计。但是放大也是可能的。在大多数国家中，有可能获得大约+20 dBm的输出功率。在欧洲，它被调低到+10 dBm左右。但是，这足以使您到达大约一到三公里的距离，这取决于您的链路预算，所拥有的放大率和/或所拥有的天线的类型。

### 2.3. 数据速率
使用2.4 GHz直接序列扩频Phy或DSSS，原始比特率为250 kb / s。在现实世界中，您将看到其中的四分之一或五分之一。预期的结果与56k波特调制解调器相当。单跳链路上每秒约52700 kb。放入多跳效果后，传播所需的时间会更长一些。

### 2.4. 开阔范围
对于2.4 GHz PHY，我们提到的范围在视线范围内可能约为2公里。这是相当可观的放大，并且在大多数领域仍在法律限制内。并且由于所有的频道，您可以进行可靠的通信，从而可以确保选择不很嘈杂的频道来避免干扰。现在，如果您确实选择了一个通道而变得嘈杂，则ZigBee会通过“频率捷变”获得高水平的响应，以便某些网络管理员可以将网络移至其他渠道。2.4 GHz频谱的另一个优势是它在全球范围内可用，这意味着您的产品具有广泛的安装基础。
...
## 5. 应用层
在应用层中，可以通过实现多个端点将物理设备拆分为多个逻辑设备。

### 5.1. 端点
每个端点代表一个逻辑设备。例如，如果我们有一个带有6个插座的智能插座适配器。我们可以使用6个端点来实现它，以便我们可以分别打开/关闭每个插座。
...
### 5.2. 集群
在每个端点中，我们可以配置几个集群。Zigbee集群实际上是一种通信模型。

![zigbee](files/ZB-Zigbee-Introduction-of-Zigbee-Basic/Cluster.png)

它基于客户端/服务器模式，用于描述两个设备之间的应用协议。每个群集都有一个群集ID，该ID在Zigbee群集库（ZCL）中定义。集群中可以定义多个属性和命令。

### 5.3. 例子
让我们看一个集群的示例，以便我们可以更好地理解它。
例如，我们需要实现一个带有两个灯泡的灯。

![zigbee](files/ZB-Zigbee-Introduction-of-Zigbee-Basic/Cluster-Example.png)

我们可以在其中定义两个端点。每个端点代表一个灯泡。

对于基本功能，例如打开/关闭，我们可以使用打开/关闭群集。指示灯是服务器端，而交换机是客户端。服务器端定义了一个属性“ on / off”，指示指示灯是打开还是关闭。还定义了诸如“打开”，“关闭”，“切换”之类的命令，这些命令应从客户端发送到服务器端。

如果我们需要更多功能，例如我们需要支持亮度级别控制。我们可以使用亮度级别控制集群。在此群集中，服务器端定义了一个属性“级别”，指示灯泡的亮度。还定义了诸如“逐渐变化亮度级别”之类的命令，这些命令应从客户端发送到服务器端。

而且，如果我们需要更多功能，例如需要支持颜色控制，则可以使用颜色控制集群。
